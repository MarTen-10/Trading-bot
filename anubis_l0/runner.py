...
def run(simulate_overrun: bool = False):
    ...
    if simulate_overrun:
        steps = [f"tool_{i}" for i in range(25)]
    ...
    for idx, step in enumerate(steps, start=1):
        ...
        if cached is not None:
            log.event("action_end", step_id=step_id, action_type="tool", tool=step, cache_hit=True, input_bytes=len(step), output_bytes=len(str(cached)), tokens_in=0, tokens_out=0, cost_estimate=0.0, latency_ms=0)
            continue
